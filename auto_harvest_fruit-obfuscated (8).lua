...
local function buildGui()
    ...
    pcall(function()
        ...
        mailQtyS = mailT:AddInput("ahf_mailqty", { Title = "Send quantity (per item)", Description = "How many of EACH picked item to send per batch (1-20). Tap to type.", Default = tostring(State.mailQty or 1), Placeholder = "1", Numeric = true, Finished = true,
            Callback = function(v) if uiBuilding then return end; State.mailQty = math.clamp(math.floor(tonumber(v) or 1), 1, 20); saveConfig() end })
    ...
    pcall(function() if mailQtyS then mailQtyS:SetValue(tostring(State.mailQty or 1)) end end)
